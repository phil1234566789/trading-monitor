...
| Wird ein bereits bestätigter Nested-Downtrend durch einen ECHTEN neuen Bruch von `nestedTrend.currRange.high` invalidiert (`applyInnerMarketStructurePivotCore`, direction='down', HIGH-Zweig), dient der auslösende Pivot selbst — als `type:'low'` umetikettiert — als Platzhalter für den neuen Origin-Low (NICHT der alte, chronologisch VOR diesem Pivot liegende `currRange.low`). Bug-Report Philip 2026-07-25 ("uptrend gebrochen -> trend:'unknown' statt Promotion, obwohl vorher ein nestedTrend existiert haben muss — beißt sich mit dem Ursprung des Algos"): der alte Low hätte die bärische Eligibility-Regel "High VOR Low" (`tryConfirmTrend`, direction='down') verletzt und eine erneute Bestätigung für den Rest der Laufzeit blockiert. Der Platzhalter (lowTime===highTime, noch nicht eligible) wird vom nächsten echten Low-Pivot automatisch auf einen späteren Zeitpunkt nachgezogen — danach ist eine erneute Bestätigung wieder möglich. | `marketStructureAnalysisChoch.test.js`: *"ein Periode-2-Pivot invalidiert einen bereits bestätigten Nested-Trend..."*, *"nach der Platzhalter-Invalidierung reicht ein einzelner weiterer Low-Pivot..."* |
