...
| Regel | Test |
|---|---|
| `nestedTrend.nestedTrend` kann selbst wieder gesetzt sein — derselbe `advanceNestedTrend`-Mechanismus läuft jetzt auch auf einem bereits bestätigten Nested-Tracker, mit demselben Pivot, den auch die darüberliegende Ebene gerade verarbeitet. | `marketStructureAnalysisNestedNestedChoch.test.js`: *"baut Ebene 3 automatisch auf, sobald Ebene 2 bestätigt ist..."* |
| Eine Promotion übernimmt eine bereits bestätigte tiefere Ebene als neue nächsttiefere Ebene (`nestedTrend: nested.nestedTrend`), statt sie wie vorher (implizit, weil strukturell unmöglich) zu verwerfen — inklusive erneuter Prüfung des brechenden Pivots gegen diese Ebene, genau wie beim Übergang Ebene 1↔2. | `marketStructureAnalysisNestedNestedChoch.test.js`: *"Promotion übernimmt eine bereits bestätigte dritte Ebene als neue zweite Ebene..."* |

| Regel | Test |
|---|---|
| Läuft über Outer-(Periode-5-) UND Periode-2-Pivots, beide mit derselben Seed-/Reseed-/Verfeinerungs-Logik (`advanceNestedTrend`, `inner=true` für P2) — und nur, solange der Haupttrend bereits bestätigt ist. Seit dem M5-Trend (Philips CHoCH vom 09.09.2026, M5, lag auf einem reinen P2-Tief 1,35576) darf auch ein P2-Pullback den Kandidaten starten und ein P2-Extrem ihn verwerfen; vorher verfeinerte P2 nur einen schon von P5 geseedeten Tracker. Gilt für 1h UND M5 (Philip will 1h ggf. später zurücknehmen). | `marketStructureAnalysisChoch.test.js`: *"ohne Nested-Trend seedet ein P2-Pullback den Tracker..."*, *"ein Periode-2-Pivot mit neuem Haupttrend-Hoch verwirft..."*, *"nach dem Verwerfen seedet ein P2-Pullback neu..."*; `m5TrendStructure.test.js`: *"M5-CHoCH am 09.09.2026..."* |
...
- `protected-high`/`LQ-sweep`/`break-of-structure` DES NESTED-TRACKERS (Chat 2026-07-25, Bug-Report
  ...
  `advanceNestedTrend`) lief hierfür schon von Anfang an mit — nur die
...
- **Seit Chat 2026-08-09 (beliebige Verschachtelungstiefe, siehe Abschnitt oben) sind die beiden
  Punkte darüber EIN gemeinsamer Codepfad, nicht mehr zwei feste Blöcke**: `renderNestedLevel`
  (parametrisiert über `isDown = nested.trend === 'downtrend'`) zeichnet genau das, was vorher die
  zwei ~80-Zeilen-Blöcke einzeln taten, aufgerufen in einer Schleife über
  `collectNestedChain(state).slice(1)` — läuft also über JEDE bestätigte Ebene (`state.nestedTrend`,
  `state.nestedTrend.nestedTrend`, ...), nicht mehr nur über `state.nestedTrend` selbst. Eine dritte
  Ebene ("CHoCH vom CHoCH") bekommt dadurch automatisch dieselbe Verbindungslinie/protected-Pivot/
  LQ-Sweep/BOS/CHoCH-Darstellung wie die zweite, ohne eigenen Code.
