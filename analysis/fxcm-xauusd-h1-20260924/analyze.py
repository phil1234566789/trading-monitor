"""Lokale Prüfung des isolierten H1-Abrufs; keinerlei Netz- oder DB-Zugriff."""
import csv
import json
import math
...
for symbol, dataset in data['instruments'].items():
    ...
    gaps = [{'from_berlin': a.astimezone(berlin).isoformat(), 'to_berlin': b.astimezone(berlin).isoformat(), 'start_distance_hours': (b-a).total_seconds()/3600} for a,b in zip(times,times[1:]) if (b-a).total_seconds() != 3600]
    ...
    summary['instruments'][symbol] = {
        ...
        'outside_range_or_open': sum(t < datetime.fromisoformat(data['start_utc']) or t.timestamp()+3600 > datetime.fromisoformat(data['end_exclusive_utc']).timestamp() for t in times),
    ...
    with (root / (symbol.replace('/','')+'-H1.csv')).open('w',newline='') as f:
...
common = sorted(set(tr_by_symbol['XAU/USD']) & set(tr_by_symbol['GBP/USD']))
x = median(tr_by_symbol['XAU/USD'][t] for t in common)
g = median(tr_by_symbol['GBP/USD'][t] for t in common)
summary['matched_h1_scale'] = {'bars':len(common),'gold_median_tr':x,'gbp_median_tr':g,'price_distance_ratio':x/g,
    'gold_h1_fvg_candidate': math.ceil(0.00015*x/g/0.01)*0.01}
