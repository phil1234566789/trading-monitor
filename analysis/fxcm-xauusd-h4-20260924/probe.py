...
def main():
    ...
    start = datetime(2026, 8, 23, 22, tzinfo=timezone.utc)
    end = datetime(2026, 9, 24, 21, tzinfo=timezone.utc)
    result = {'start_utc': start.isoformat(), 'end_exclusive_utc': end.isoformat(),
              'timeframe': 'H4', 'side': 'Bid', 'instruments': {}, 'offers': {}}
    os.chdir('/tmp')
    with ForexConnect() as fx:
        ...
        for symbol in ('XAU/USD', 'GBP/USD'):
            ...
            while cursor < end:
                ...
                history = fx.get_history(symbol, 'H4', cursor, stop)
                rows = closed_rows(symbol.replace('/', ''), '4h', history, end.timestamp())
