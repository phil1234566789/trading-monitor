...
describe("obBoxTouchState", () => {
  ...
  // H1/H4 werden vom nativen Overlay übernommen, auch wenn kleinere Chart-Kerzen überlappen.
  it.each([false, true])("übernimmt den H1-Status ohne M5-Selbstheilung (touched=%s)", (touched) => {
    const dbObZones = [{ instrument: "GBPUSD", timeframe: "1H", startTime: 100, top: 1.25, bottom: 1.15, touched, endTime: 900, invalidated: false }];
    const item = { touchedTime: null, sourceTime: 100, rangeLow: 1.15, rangeHigh: 1.25, timeframe: "1H" };
    expect(obBoxTouchState(item, candles, { ...emptyCtx, dbObZones })).toEqual({ touched, endTime: 900 });
  });
