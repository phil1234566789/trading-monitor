...
something is a bug. `git log -p -- AGENTS.md` recovers the fuller history/dates/quotes behind any
...
**When Codex transcribes/seeds a timestamp itself** (e.g. a one-off data migration entering
...
`.Codex/commands/l.md` switches a session from this file's normal coding-assistant behavior into
"Laniakea", Philip's trading-sparring-partner persona (Bias/Setup-analysis, not code) — pointer to
`trading/Codex-project-instructions.md` (the same persona instructions used in Philip's separate
Codex.ai Project) plus a note to use this repo's MCP tools for live data instead of waiting for
...
- **When Philip corrects a mistake in how Codex worked (not a trading-analysis call, but the
  ...
  belongs wherever it will structurally take effect next time: this AGENTS.md itself, an MCP
...
- **A comment block over ~4-5 lines needs a real reason** (e.g. several independent bug reports
  ...
  elsewhere (link to it instead of restating it).
- When explaining a detection-algorithm bug that hinges on price levels/timing (why a setup did
  or didn't fire, why a sweep did or didn't count), default to a Codex Artifact with an annotated
