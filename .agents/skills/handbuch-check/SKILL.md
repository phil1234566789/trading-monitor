...
`Write|Edit`, siehe `.Codex/settings.local.json`) erinnert daran im letztmöglichen Moment — spätestens
...
6. **Kein "Bug-Beispiel DATUM (...)" in unconditional geladenen Dateien.** `00-trading-steps/**`,
   ...
   AGENTS.md, Abschnitt "Ladekosten"). Ein neuer Regel-Satz reicht meist mit EINEM knappen Halbsatz
